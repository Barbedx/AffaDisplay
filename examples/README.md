...
| example | board | what it is |
|---|---|---|
| `17_mediascreen` | DevKit V1 | **Both layers at once.** A live 48×48 spectrum, clock and progress bar in the `0x1F1` nav pane while `setText` marquees the track title on the main line. The first thing here that could not be built before `16_navlab`: it needs the pane to render, to be an *independent layer* rather than a screen mode, and to be safe to animate. Drawn on the device into a double buffer — a clock that advances is not knowable at build time, and `showNavBitmap()` borrows its bytes until the ticket completes. Measured on the bench: **64 ms an image, 23% bus duty at 4 fps, 141/141 frames with zero errors.** The console reports duty and dropped frames so the frame-rate trade is visible rather than guessed. |
...
| `16_navlab` | **What are the fourteen header bytes in front of the `0x1F1` nav bitmap?** The OEM's 302-byte nav screen decodes as a 14-byte header plus a 48×48 monochrome bitmap, and the bitmap *is* the idle globe (`../docs/PROTOCOL-NOTES.md` §18). This replays it byte for byte, makes every byte editable from a browser canvas, ships alternative images (Renault losange, tryzub, tryzub+clock, thermometer, battery, and an orientation checker), renders arbitrary text through the browser's own font, and **sweeps one header byte across a range**, one send per step, so an unknown field is characterised in a single pass instead of a rebuild per guess. First and only consumer of `enqueueExternal()`. **Nothing in it has reached glass yet** — `12_ulclock`'s lesson applies in full: this panel ACKs everything, so only the glass counts. |
